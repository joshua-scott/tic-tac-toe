...
function cpuMove() {
  ...
  if (winOrBlock('win')) return;else if (winOrBlock('block')) return;else if (goInCentre()) return;else if (coverOppositeCorners()) return;else if (coverAdjoiningEdges()) return;else if (offensiveMove()) return;else goInRemaining();
}
...
function oneEmptyOneX(board, cell1, cell2) {
  return board[cell1] === 'X' && board[cell2] === '' || board[cell1] === '' && board[cell2] === 'X';
}
